...
func TestPancakeProvider_GetTickerPrices(t *testing.T) {
	p, err := NewPancakeProvider(
		context.TODO(),
		zerolog.Nop(),
		Endpoint{},
		OSMOATOM,
...
func TestPancakeProvider_GetCandlePrices(t *testing.T) {
	p, err := NewPancakeProvider(
		context.TODO(),
		zerolog.Nop(),
		Endpoint{},
...
func TestPancakeCurrencyPairToPancakePair(t *testing.T) {
	cp := types.CurrencyPair{Base: "ATOM", Quote: "USDT"}
	pancakeSymbol := currencyPairToPancakePair(cp)
	require.Equal(t, pancakeSymbol, "ATOM/USDT")
}
