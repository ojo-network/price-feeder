...
func TestCamelotProvider_GetTickerPrices(t *testing.T) {
	p, err := NewCamelotProvider(
		context.TODO(),
		zerolog.Nop(),
		Endpoint{},
		OSMOATOM,
...
func TestCamelotProvider_GetCandlePrices(t *testing.T) {
	p, err := NewCamelotProvider(
		context.TODO(),
		zerolog.Nop(),
		Endpoint{},
...
func TestCamelotCurrencyPairToCamelotPair(t *testing.T) {
	cp := types.CurrencyPair{Base: "ATOM", Quote: "USDT"}
	camelotSymbol := currencyPairToCamelotPair(cp)
	require.Equal(t, camelotSymbol, "ATOM/USDT")
}
