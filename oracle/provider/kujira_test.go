...
func TestKujiraProvider_GetCandlePrices(t *testing.T) {
	...
	t.Run("valid_request_single_candle", func(t *testing.T) {
		...
		require.NoError(t, err)
		require.Len(t, prices, 1)
		require.Equal(t, math.LegacyMustNewDecFromStr(price), prices[KUJIATOM][0].Price)
		require.Equal(t, math.LegacyMustNewDecFromStr(volume), prices[KUJIATOM][0].Volume)
