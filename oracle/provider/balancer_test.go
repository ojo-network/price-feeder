...
func TestBalancerProvider_GetTickerPrices(t *testing.T) {
	p, err := NewBalancerProvider(
		context.TODO(),
		zerolog.Nop(),
		Endpoint{},
		OSMOATOM,
...
func TestBalancerProvider_GetCandlePrices(t *testing.T) {
	p, err := NewBalancerProvider(
		context.TODO(),
		zerolog.Nop(),
		Endpoint{},
...
func TestBalancerCurrencyPairToBalancerPair(t *testing.T) {
	cp := types.CurrencyPair{Base: "ATOM", Quote: "USDT"}
	balancerSymbol := currencyPairToBalancerPair(cp)
	require.Equal(t, balancerSymbol, "ATOM/USDT")
}
